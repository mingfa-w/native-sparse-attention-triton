...
if __name__ == "__main__":
    ...
    block_size = 64
    topk = 16
    seqlens = torch.LongTensor([1000, 4000, 8192]).int().npu()
    ...
    q = (
        torch.empty(cu_seqlens[-1], num_heads, head_dim, device="npu")
        .uniform_(-1, 1)
        .to(torch.float16)
    )
    k = (
        torch.empty(cu_seqlens[-1], num_heads // 4, head_dim, device="npu")
        .uniform_(-1, 1)
        .to(torch.float16)
    )
    v = (
